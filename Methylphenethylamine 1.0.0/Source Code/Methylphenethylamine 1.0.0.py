...
import os
from Audio.AudioStretch import AudioStretch
...
class AudioStretchGUI:
    ...
    def run_stretch(self):
        ...
        try:
            ...
            if mode == -1:
                modified_samples = AudioStretch.slow([samples], 1)
            else:
                modified_samples = AudioStretch.fast([samples], mode)
...
if __name__ == "__main__":
    root = tk.Tk()
    gui = AudioStretchGUI(root)
    root.mainloop()

#Exe string in command prompt: pyinstaller --onefile --windowed --hidden-import=Audio.AudioStretch --hidden-import=Audio.AudioSampleValues --add-data "Background.png;." "Methylphenethylamine 1.0.0.py"